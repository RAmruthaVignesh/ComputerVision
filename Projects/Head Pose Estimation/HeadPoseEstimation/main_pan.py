...
labels= list(np.genfromtxt(label_csv_path, delimiter=',', skip_header=1, usecols=6, dtype=None))
...
network = model_sanket.cnn_model(X)
...
for epoch in range(10000):
    ...
    if (epoch % 10) == 0:
        ...
        for class_label in range(3):
            class_corr_pred = tf.equal(pred==class_label,train_Y==class_label)
            class_accuracy = tf.reduce_mean(tf.cast(class_corr_pred,tf.float32))
            print("Accuracy after epoch {} for class_label {} is {}".format(epoch, class_label, sess.run(class_accuracy)))
